...
CREATE TABLE IF NOT EXISTS policy.member (
...
CREATE INDEX IF NOT EXISTS idx_member_number ON policy.member(member_number);
CREATE INDEX IF NOT EXISTS idx_member_state ON policy.member(state);
CREATE INDEX IF NOT EXISTS idx_member_dob ON policy.member(date_of_birth);
...
CREATE TABLE IF NOT EXISTS policy.member_update (
...
CREATE INDEX IF NOT EXISTS idx_member_update_member ON policy.member_update(member_id);
CREATE INDEX IF NOT EXISTS idx_member_update_date ON policy.member_update(change_date);
CREATE INDEX IF NOT EXISTS idx_member_update_type ON policy.member_update(change_type);

-- APPLICATION: New policy applications
CREATE TABLE IF NOT EXISTS policy.application (
...
CREATE INDEX IF NOT EXISTS idx_application_number ON policy.application(application_number);
CREATE INDEX IF NOT EXISTS idx_application_status ON policy.application(application_status);
CREATE INDEX IF NOT EXISTS idx_application_submission ON policy.application(submission_date);

-- APPLICATION_MEMBER: Members on application
CREATE TABLE IF NOT EXISTS policy.application_member (
    application_member_id   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    application_id          UUID NOT NULL REFERENCES policy.application(application_id),
    ...
    email                   VARCHAR(200),
    mobile_phone            VARCHAR(20),
    existing_member_id      UUID REFERENCES policy.member(member_id),

    created_at              TIMESTAMP NOT NULL DEFAULT NOW(),
    created_by              VARCHAR(50) NOT NULL DEFAULT 'SIMULATION'
);

CREATE INDEX IF NOT EXISTS idx_appmember_application ON policy.application_member(application_id);

-- POLICY: Insurance policies
CREATE TABLE IF NOT EXISTS policy.policy (
    policy_id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    policy_number           VARCHAR(25) NOT NULL UNIQUE,
    application_id          UUID REFERENCES policy.application(application_id),
    product_id              INTEGER NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_policy_number ON policy.policy(policy_number);
CREATE INDEX IF NOT EXISTS idx_policy_status ON policy.policy(policy_status);
CREATE INDEX IF NOT EXISTS idx_policy_product ON policy.policy(product_id);
CREATE INDEX IF NOT EXISTS idx_policy_effective ON policy.policy(effective_date);

-- POLICY_MEMBER: Links members to policies with roles
CREATE TABLE IF NOT EXISTS policy.policy_member (
...
CREATE INDEX IF NOT EXISTS idx_pm_policy ON policy.policy_member(policy_id);
CREATE INDEX IF NOT EXISTS idx_pm_member ON policy.policy_member(member_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_pm_active ON policy.policy_member(policy_id, member_id)
...
CREATE INDEX IF NOT EXISTS idx_coverage_policy ON policy.coverage(policy_id);
CREATE INDEX IF NOT EXISTS idx_coverage_type ON policy.coverage(coverage_type);

-- WAITING_PERIOD: Waiting periods by coverage type per member
CREATE TABLE IF NOT EXISTS policy.waiting_period (
...
CREATE INDEX IF NOT EXISTS idx_wp_policy_member ON policy.waiting_period(policy_member_id);
CREATE INDEX IF NOT EXISTS idx_wp_coverage ON policy.waiting_period(coverage_id);
CREATE INDEX IF NOT EXISTS idx_wp_status ON policy.waiting_period(status);
CREATE INDEX IF NOT EXISTS idx_wp_end_date ON policy.waiting_period(end_date);

-- HEALTH_DECLARATION: Health questions and declarations
CREATE TABLE IF NOT EXISTS policy.health_declaration (
    health_declaration_id   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    application_member_id   UUID NOT NULL REFERENCES policy.application_member(application_member_id),
    application_id          UUID NOT NULL REFERENCES policy.application(application_id),
...
CREATE INDEX IF NOT EXISTS idx_hd_application ON policy.health_declaration(application_id);
