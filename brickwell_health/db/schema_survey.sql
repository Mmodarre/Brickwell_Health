...
CREATE TABLE IF NOT EXISTS survey.nps_survey_pending (
    -- Primary Key
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_nps_pending_status ON survey.nps_survey_pending(processing_status);
CREATE INDEX IF NOT EXISTS idx_nps_pending_member_date ON survey.nps_survey_pending(member_id, simulation_date);
CREATE INDEX IF NOT EXISTS idx_nps_pending_will_respond ON survey.nps_survey_pending(will_respond)
    WHERE will_respond = TRUE;
CREATE INDEX IF NOT EXISTS idx_nps_pending_trigger ON survey.nps_survey_pending(trigger_event);
...
CREATE TABLE IF NOT EXISTS survey.nps_survey (
    -- Primary Key
    ...
    CONSTRAINT fk_nps_pending FOREIGN KEY (pending_id)
        REFERENCES survey.nps_survey_pending(pending_id)
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_nps_member ON survey.nps_survey(member_id);
CREATE INDEX IF NOT EXISTS idx_nps_policy ON survey.nps_survey(policy_id);
CREATE INDEX IF NOT EXISTS idx_nps_sent ON survey.nps_survey(sent_date);
CREATE INDEX IF NOT EXISTS idx_nps_score ON survey.nps_survey(nps_score) WHERE nps_score IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_nps_category ON survey.nps_survey(nps_category) WHERE nps_category IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_nps_claim ON survey.nps_survey(claim_id) WHERE claim_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_nps_detractors ON survey.nps_survey(nps_category) WHERE nps_category = 'Detractor';
CREATE INDEX IF NOT EXISTS idx_nps_trigger ON survey.nps_survey(trigger_event);
...
CREATE TABLE IF NOT EXISTS survey.csat_survey_pending (
    -- Primary Key
    pending_id UUID PRIMARY KEY,
    survey_reference VARCHAR(30) NOT NULL,
    ...
    CONSTRAINT fk_csat_pending_case FOREIGN KEY (case_id)
        REFERENCES crm.service_case(case_id)
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_csat_pending_status ON survey.csat_survey_pending(processing_status);
CREATE INDEX IF NOT EXISTS idx_csat_pending_member_date ON survey.csat_survey_pending(member_id, simulation_date);
CREATE INDEX IF NOT EXISTS idx_csat_pending_will_respond ON survey.csat_survey_pending(will_respond)
    WHERE will_respond = TRUE;
...
CREATE TABLE IF NOT EXISTS survey.csat_survey (
    -- Primary Key
    survey_id UUID PRIMARY KEY,
    survey_reference VARCHAR(30) NOT NULL UNIQUE,
    ...
    CONSTRAINT fk_csat_pending FOREIGN KEY (pending_id)
        REFERENCES survey.csat_survey_pending(pending_id)
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_csat_member ON survey.csat_survey(member_id);
CREATE INDEX IF NOT EXISTS idx_csat_interaction ON survey.csat_survey(interaction_id)
    WHERE interaction_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_csat_case ON survey.csat_survey(case_id) WHERE case_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_csat_score ON survey.csat_survey(csat_score) WHERE csat_score IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_csat_sent ON survey.csat_survey(sent_date);
